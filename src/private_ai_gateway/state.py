...
def open_backend(config: StateConfig, environ: Any | None = None) -> OpenedBackend:
    ...
    try:
        if config.evidence_mode == EVIDENCE_MODE_DURABLE:
            ...
            # Step 7B.2: with both stores open and each independently integrity-validated,
            # run the single cross-store reconciliation pass before the runtime begins. It
            # subsumes the 7B.1 reserved-but-unconsumed resolver as class 2 — deliberately
            # not a separate repair beforehand, which would have erased the original
            # cross-store shape the general classifier needs to see.
            try:
                reconciliation.reconcile(authority_store, evidence_sink)
            except reconciliation.ReconciliationError as exc:
                # Being unable to inspect or safely repair the cross-store state is an
                # unsafe startup condition, never a clean one. Surfaced as StateError so
                # every fail-closed startup path has one type.
                raise StateError(
                    f"startup cross-store reconciliation failed closed: {exc}"
                ) from exc
        else:
